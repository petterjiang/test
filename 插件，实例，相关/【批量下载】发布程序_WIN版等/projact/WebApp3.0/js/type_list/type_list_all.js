...
function load_dev_list() {
	function GetUiDevList(data) {
		$.each(data, function(i, n) {
			...
			var html_drag = '<div class="drag" ' + style + ' title="左键拖动，右键查看具体信息" ui_dev_type="' + ui_dev_type + '" place_sts="' + place_sts + '" in_map="' + in_map + '" x_posent="' + area_x + '" y_posent="' + area_y + '" ui_dev_id="' + ui_dev_id + '"><div class="img_box tuodong"><img src="../' + file_path + '" width="40" style="display:none;"><div class="s_s"><p class="zhuantai ' + zhuangtaiClass + '"><span></span>' + zhuangtaiTexgt + '</p><div class="ctrlkg" title="布撤防按钮"><span class="' + buchefangClass + '"></span></div><div class="button_kz"></div></div></div><a href="javascript:void(0);" class="handle_bott">' + ui_dev_name + '</a></div>'
...
function full_map() {
	...
	$(".user_mssage .top_width").css('top', '-40px');
	$(".move_text").css('position', 'fixed');
...
function exit_full_map() {
	...
	$(".user_mssage .top_width").css('top', '0');
	$(".move_text").css('position', 'relative');
...
function move_text() {
	var lunbo = setInterval(function() {
		var top = $(".movetext").position().top
		li_lenth = $(".movetext li").length
		// alert(li_lenth)
		if (li_lenth > 0) {
			$(".movetext").stop(true, true).animate({
				"top": "-22px"
			}, 800, function() {
				$(".movetext li").first().remove();
				$(".movetext").css('top', 0);
			})
		};
	}, 3000)
...
$(function() {
				...
				obj.find("li").each(
					function() {
						if ($(this).attr("ui_prop_id") == ui_prop_id) {
							...
							var val_name = $(this).find('.name_shuxing').text()
							if ($(".move_text .movetext li").length < 2) {
								$(".move_text .movetext").append('<li>' + dev_name + '的' + val_name + '属性值变更为' + prop_value + '</li>')
							};
						}
					}
				);
	...
	function zhuangtaitu(ui_dev_id) {
		...
		function tpye_in(data) {
			$.each(data, function(i, o) {
				var ui_dev_type = o.ui_dev_type //:应用设备类型
				var ui_dev_type_name = o.ui_dev_type_name //:应用设备类型名称
				if ($(".drag[ui_dev_type='" + ui_dev_type + "']").length > 0) {
					$(".leixing ul").append('<li pro_value="' + ui_dev_type + '">' + ui_dev_type_name + '</li>')
				};

			});
		}
		DataHandle("GetUiDevTypeList", {}, tpye_in, loading)
		...
		function alert_load(data) { //读取告警输出html
			$.each(data, function(o, n) {
				...
				if (prop_type.indexOf("r") > -1) {
					if (alarm_cond == 0) {
						add_con = "<tr ui_dev_id='" + ui_dev_id + "' ui_prop_id = '" + ui_prop_id + "' cond='" + alarm_cond + "'><td class='name_pro' width=12% height=30>" + ui_prop_name + "</td><td width=60% colspan=5 class='cont'>暂无告警配置</td><td><a class='gjpz'>配置告警</a></td></tr>";
					} else {
						add_con = "<tr ui_dev_id='" + ui_dev_id + "' ui_prop_id = '" + ui_prop_id + "' cond='" + alarm_cond + "'><td class='name_pro' width=12% height=30>" + ui_prop_name + "</td><td width=12% class='alarm_cond'>" + alarm_cond_text + "</td><td width=12% class='alarm_value'>" + alarm_value + "</td><td width=12% class='pre_time'>" + pre_time + "(秒)</td><td width=12% class='send_time'>" + send_times + "</td><td width=12% class='send_interval'>" + send_interval + "(分钟)</td><td><a class='gjpz'>修改配置</a></td></tr>";
					}
